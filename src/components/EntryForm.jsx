...
function EntryForm({ onSubmit, onCancel, editEntry = null }) {
  ...
  // Initialize selectedFiles from metadata when editing
  const getInitialFiles = () => {
    if (metadata.files && Array.isArray(metadata.files)) {
      return metadata.files.map(f => ({
        name: f.fileName,
        type: f.fileType || 'application/octet-stream',
        isExisting: true // Mark as existing file (already saved)
      }));
    }
    return [];
  };
  const [selectedFiles, setSelectedFiles] = useState(getInitialFiles);

  // Action Items fields
  const [actionItemsDescription, setActionItemsDescription] = useState(metadata.description || '');
  const [actionItems, setActionItems] = useState(metadata.items || []);
  ...
  const handleBrowseFile = async () => {
    try {
      ...
      if (file) {
        ...
        console.log('File added:', { fileName: file.name, fileType: file.type });
      }
    } catch (error) {
  ...
  const handleAddActionItem = () => {
    if (newActionItemText.trim()) {
      setActionItems([...actionItems, { text: newActionItemText.trim(), completed: false }]);
    ...
    switch (entryType) {
      ...
      case 'file':
        ...
        if (selectedFiles.length > 1) {
          ...
          autoGeneratedTitle = selectedFiles[0].name;
        }
        break;

      case 'action_items':
        if (actionItems.length === 0) {
          alert('Please add at least one action item');
          return;
        }
        newMetadata = {
          description: actionItemsDescription.trim(),
          items: actionItems,
        };
        autoGeneratedTitle = `Action Items${actionItemsDescription ? ': ' + actionItemsDescription.substring(0, 40) : ''}`;
        break;
    }
        ...
        return (
          <>
            <div className="form-group">
              <label>Description</label>
              <textarea
                value={actionItemsDescription}
                onChange={(e) => setActionItemsDescription(e.target.value)}
                placeholder="What are these action items for?"
                rows="3"
              />
            </div>
            <div className="form-group">
